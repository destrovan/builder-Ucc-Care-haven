...
export const submitReport: RequestHandler = async (req, res) => {
  try {
    const { urgency, description, contactPreference, isAnonymous, studentId } = req.body;
    
    if (!description || !urgency) {
      return res.status(400).json({ error: 'Description and urgency are required' });
    }
    ...
    const reportResult = await query(
      ...
      [trackingId, isAnonymous ? null : studentId, isAnonymous, urgency, description, contactPreference]
    );
    ...
    await query(
      ...
      [report.id, 'Your support request has been received and is being reviewed by our professional team.']
    );
  ...
  try {
    ...
    res.json({
      ...
      report: {
        ...
        counselor: report.first_name && report.last_name ? 
          `${report.first_name} ${report.last_name}` : 'Unassigned',
      ...
      updates: updatesResult.rows.map(update => ({
        ...
        author: update.first_name && update.last_name ? 
          `${update.first_name} ${update.last_name}` : 'System'
      }))
    });
...
export const getCounselorCases: RequestHandler = async (req, res) => {
  try {
    ...
    const cases = casesResult.rows.map(case_ => ({
      ...
      student: case_.is_anonymous ? 'Anonymous Student' : 
